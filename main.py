...
def main(config):
    ...
    for epoch in range(config.TRAIN.START_EPOCH, config.TRAIN.EPOCHS):
        ...
        if dist.get_rank() == 0 and (hter < max_accuracy):
            max_accuracy = hter
            max_epoch = epoch
        ...
        logger.info(f"Minimal HTER of the network on the oulu test images: {max_accuracy:.4f} with epoch #{max_epoch}.")
...
def train_one_epoch(config, model, criterion, data_loader, optimizer, epoch, lr_scheduler):
    ...
    for idx, (samples, targets) in enumerate(data_loader):
        ...
        if config.TRAIN.ACCUMULATION_STEPS > 1:
            loss = criterion(outputs, targets) + (color_dis_p + color_dis_n)*0.5 + loss_center*0.5
            # loss = criterion(outputs, targets) + loss_center
            # loss = loss_center
...
@torch.no_grad()
def validate_intra(config, data_loader, model):
    losses   = []
    corrects = []
    probs = []
    labels = []
    ...
    for idx, (images, target) in enumerate(data_loader):
        ...
        images = images.view(48,9,3,48,48)
        ...
        with torch.no_grad():
            ...
            thres = 0.8 # procotol 3&4 : 0.9; protocol 2: 0.5
            if pred > thres:
                if target == 1:
                    tp = tp + 1
                else:
